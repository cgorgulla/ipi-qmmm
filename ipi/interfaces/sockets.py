...
from __future__ import print_function                    # c*g
import sys
import os
import socket
import select
import string
import time
from ipi.utils.softexit import softexit                    # c*g
...
def eprint(*args, **kwargs):                    # c*g
    print(*args, file=sys.stderr, **kwargs)     # c*g
      ...
      for c in self.clients[:]:
         if not (c.status & Status.Up):
            try:
               ...
               softexit.trigger("Error: One client has died unexpectedly. Exiting i-PI...")  # c*g
               eprint("Error: One client has died unexpectedly. Exiting i-PI...")            # c*g
               c.shutdown(socket.SHUT_RDWR)
               c.close()
            except socket.error:
               pass
