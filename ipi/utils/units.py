...
class Elements(dict):
    ...
    def mass(cls, label):
        ...
        try:
            label=label[0]+label[1:].lower() # making all letters lower case except first one # me c*g
            return cls.mass_list[label] * Constants.amu
        except KeyError:
            info("Unknown element given (" + label + "), you must specify the mass", verbosity.low)
            return -1.0
